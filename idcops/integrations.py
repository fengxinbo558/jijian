        ...
        built_in.extend(
            [
                _status(
                    "linux_otel_agent",
                    "Linux 日志与主机指标",
                    "由 OpenTelemetry Collector 读取 journald、系统日志和主机指标并送入 SigNoz",
                    "planned",
                    "仓库已有单机采集模板；需要客户在获准服务器部署并填写完整 SN、机架位和机房编码",
                    False,
                    True,
                ),
                _status(
                    "network_syslog_nms",
                    "网络 syslog 与 NMS",
                    "交换机主动发送 syslog，已有网络监控通过 SNMP/Telemetry 形成告警后再送入机鉴",
                    "planned",
                    "接口与模拟测试已准备；尚未连接客户交换机、syslog 中心或网络监控平台",
                    False,
                    True,
                ),
                _status(
                    "snmp_redfish",
                    "BMC 与硬件健康",
                    "通过 Redfish、IPMI 或现有硬件监控读取传感器、SEL 和设备健康状态",
                    "planned",
                    "本轮保留只读接口，尚未连接真实 BMC",
                    False,
                    True,
                ...
                _status(
                    "dcim_environment",
                    "动环 / DCIM 告警",
                    "接收温度、供电、漏水和烟雾等设施告警，再结合机房等级和实际影响判断 CC",
                    "planned",
                    "设施事件矩阵与测试已完成；尚未连接客户动环或 DCIM",
                    False,
                    True,
                ),
                _status(
