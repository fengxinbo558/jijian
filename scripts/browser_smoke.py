...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        page.get_by_role("heading", name="故障中心").wait_for()

        page.get_by_role("button", name="运行模拟案例").click()
        page.get_by_role("button", name="运行这个场景").first.click()
        page.locator(".incident-row").first.wait_for()
        page.get_by_role("tab", name="处置步骤").click()
        ...
        page.get_by_role("tab", name="审计记录").click()
        ...
        page.get_by_role("tab", name="AI 分析与证据").click()
        ...
        page.get_by_role("button", name="查看数据来源").click()
        ...
        page.get_by_role("button", name="真实故障", exact=True).click()
        ...
        page.locator(".identity-strip").get_by_text(browser_sn, exact=True).wait_for()
        assert browser_sn in page.locator(".identity-strip").inner_text()
        page.get_by_role("tab", name="原始日志").click()
        ...
        assert "尚未通过真实工具或人工检查确认" in page.locator(".judgment-glance").inner_text()
        page.get_by_role("tab", name="审计记录").click()
        page.get_by_role("button", name="查询监控并让 AI 补充调查").click()
        page.get_by_role("tab", name="AI 分析与证据").click()
        page.wait_for_function("() => document.querySelector('.investigation-section')?.textContent?.includes('SigNoz 尚未连接，未执行自动日志查询')")
        page.get_by_role("tab", name="处置步骤").click()
        ...
        mobile.locator(".incident-row").first.click()
        mobile.get_by_role("tab", name="审计记录").click()
        mobile.locator(".capability-banner").wait_for()
        mobile.get_by_role("tab", name="AI 分析与证据").click()
